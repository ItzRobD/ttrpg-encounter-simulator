...
func TestGetWeaponAttackData_ModifiersAndVersatile(t *testing.T) {
	...
	rapier := &weapon.Weapon{
		Name: "Rapier",
		DamageBlocks: []core.DamageBlock{
			{NumberOfDice: 1, Die: core.D8, DamageType: core.DamagePiercing},
		},
		Properties: weapon.Properties{
			IsRanged:  false,
			IsFinesse: true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.versatile && ad.GetDie() != tt.w.DamageBlocks[0].Die+2 {
				t.Errorf("Versatile die should be +2: got %v", ad.GetDie())
			}
		})
	}
}
...
func TestGetWeaponAttackData_Versatile(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if ad.GetDie() != tt.wantDie {
				t.Errorf("Die = %v, want %v", ad.GetDie(), tt.wantDie)
			}
...
func TestComputeAttackDataForSlot(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if ad.GetNumberOfDice() != tt.weapon.DamageBlocks[0].NumberOfDice {
				t.Errorf("NumberOfDice = %d, want %d", ad.GetNumberOfDice(), tt.weapon.DamageBlocks[0].NumberOfDice)
			}

			if ad.GetDie() != tt.weapon.DamageBlocks[0].Die {
				t.Errorf("Die = %v, want %v", ad.GetDie(), tt.weapon.DamageBlocks[0].Die)
			}
		})
	}
}
