...
func getArmorByID(ctx context.Context, id int) (Armor, error) {
	...
	err = row.Scan(&armorResult.ID, &armorResult.Name, &armorResult.ArmorClass, &armorResult.DexBonus, &armorResult.MaxBonus, &armorResult.MinimumStr)
	if err != nil {
...
func GetAllArmorData(ctx context.Context) ([]Armor, error) {
	var armors []Armor
	stmt := SELECT(
		EquipmentArmor.ID,
		EquipmentArmor.Name,
	...
	if err != nil {
		return armors, fmt.Errorf("error getting armor data: %w", err)
	}
	defer rows.Close()
	for rows.Next() {
		var armor Armor
		err = rows.Scan(&armor.ID, &armor.Name, &armor.ArmorClass, &armor.DexBonus, &armor.MaxBonus, &armor.MinimumStr)
		if err != nil {
			return armors, fmt.Errorf("error scanning armor data: %w", err)
		}
