...
// ChooseTargetForFollowUp selects a new target for subsequent swings in a multiattack, excluding
// any instance IDs provided in the exclude map and filtering out downed (<= 0 HP) targets.
// Returns (target, status). If no valid targets exist, status will be TargetNone.
func (mai *MonsterAI) ChooseTargetForFollowUp(exclude map[int]bool) (core.Entity, core.TargetStatus) {
	if mai.combatCtx == nil || mai.parent == nil {
		return nil, core.TargetNone
	}
	// Build list of valid opposing combatants
	bestScore := -1.0
	var bestEntity core.Entity

	for id, info := range mai.combatCtx.CombatantInfo {
		// Skip self
		if id == mai.parent.GetInstanceID() {
			continue
		}
		// Skip excluded
		...
		// Only target entities of the opposite side
		if (mai.parent.IsMonster() && info.Combatant.Entity.IsMonster()) || (mai.parent.IsCharacter() && info.Combatant.Entity.IsCharacter()) {
			continue
		}
		// Skip downed targets
		if info.State.CurrentHP <= 0 {
			continue
		}
		// Simple heuristic: prefer lowest current HP, break ties by lowest AC
		score := float64(-info.State.CurrentHP)
		if bestEntity == nil || score > bestScore {
			bestScore = score
			bestEntity = info.Combatant.Entity
		}
	}
...
func (mai *MonsterAI) createMonsterDamageActionRequest() (*core.AIRequest, error) {
	...
	if mai.parent.EntityStateManager.GetHasUsedAction() {
		// Log that we have no action left if we are debugging
		if mai.combatCtx.Options.DebugAI {
			fmt.Printf("[DEBUG AI] %s: No Action available\n", mai.parent.GetName())
		}
		return nil, nil
	}
	...
	if mai.hasMultiattack {
		...
		if err == nil {
			// In Tactical/Weighted mode, we still prefer spell if it's "better" or if simplified logic was used.
			// But for now, if it has multiattack, let's use it.
			if mai.AIMode == MAISimple && mai.parent.IsSpellcaster() {
	...
	if err != nil {
		...
		// Detect if this action is actually part of a multiattack (unlikely here but for safety)
		// Usually standard actions and multiattacks are separate IDs.
		return mai.buildAIRequest(actionChoiceID, targetID, nil, core.ATMonsterAction)
	}
}
...
func (mai *MonsterAI) chooseActionWeighted() (core.ActionType, error) {
	...
	if healUtility > damageUtility {
		...
		// In tactical mode, we check if the damage action corresponds to a multiattack
		if mai.hasMultiattack {
			_, err := mai.chooseMultiattackOption()
			if err == nil {
				chosenAction = core.ATMonsterMultiattack
			} else {
...
func (mai *MonsterAI) ChooseMonsterActionType() (core.ActionType, error) {
	...
	if mai.hasMultiattack {
		return core.ATMonsterMultiattack, nil
	}
