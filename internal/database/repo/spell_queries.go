...
func GetSpellSummaries(ctx context.Context) (map[int]spells.SpellSummary, error) {
	summaries := make(map[int]spells.SpellSummary)
	var err error
	...
	if err != nil {
		return nil, fmt.Errorf("failed to query spell summaries: %w", err)
	}
	defer rows.Close()
	for rows.Next() {
		var s spells.SpellSummary
		err = rows.Scan(
			&s.ID,
			&s.Name,
			&s.IsConcentration,
			...
			&s.HasDC)
		if err != nil {
			return nil, fmt.Errorf("failed to collect spell summaries: %w", err)
		}
...
func GetSpellSummariesByClassID(ctx context.Context, classID uint8) (map[int]spells.SpellSummary, error) {
	summaries := make(map[int]spells.SpellSummary)
	spellIDs, err := GetUsableSpellIDsByClassID(ctx, classID)
	if err != nil {
		return nil, err
	}
	...
	stmt := SELECT(
		...
		Spells.IsAoe,
		Spells.HasDc,
	).FROM(Spells).WHERE(Spells.ID.IN(util.IntsToExpressions(spellIDs)...))

	query, args := stmt.Sql()
	rows, err := database.Query(ctx, query, args...)
	if err != nil {
		return nil, fmt.Errorf("failed to query spell summaries by class id: %d - %w", classID, err)
	}
	defer rows.Close()
	for rows.Next() {
		var s spells.SpellSummary
		...
		if err != nil {
			return nil, fmt.Errorf("failed to scan spell summary row: %w", err)
		}
		summaries[s.ID] = s
	}
	return summaries, nil
...
func QuerySpellData(ctx context.Context, params spells.SpellQueryParams) (map[core.ID]spells.Spell, error) {
	var spellMap map[core.ID]spells.Spell
	var err error
	if len(params.ID) == 0 {
		...
		spellMap, err = getSpellsByID(ctx, ids)
		if err != nil {
			return nil, err
		}
	} else {
		spellMap, err = getSpellsByID(ctx, params.ID)
		if err != nil {
			return nil, err
		}
	}
...
func getSpellsByID(ctx context.Context, ids []int) (map[core.ID]spells.Spell, error) {
	if len(ids) == 0 {
		return nil, fmt.Errorf("no spell ids provided")
	}
	spellMap := make(map[core.ID]spells.Spell)
	stmt := SELECT(
	...
	for rows.Next() {
		...
		if dcOnSuccess.Valid {
			...
			if err != nil {
				return nil, fmt.Errorf("failed to collect spells: invalid dc on success returned: spellID %s", s.ID)
			}
		} else {
		...
		if formula.CastLevel != -1 {
			...
			default:
				return nil, fmt.Errorf("failed to collect spells: invalid number of dice returned: spellID %s", s.ID)
			}
			...
			default:
				return nil, fmt.Errorf("failed to collect spells: invalid amount to add returned: spellID %s", s.ID)
			}
			...
			default:
				return nil, fmt.Errorf("failed to collect spells: invalid use spellmod returned: spellID %s", s.ID)
			}
			...
			if damageType.Valid {
				...
				if err != nil {
					return nil, fmt.Errorf("failed to collect spells: invalid damage type returned: spellID %s", s.ID)
				}
			}
			...
			if err != nil {
				return nil, fmt.Errorf("failed to collect spells: unable to calculate average value for spell id: %s - %w", s.ID, err)
			}
			formula.AverageValue = avg
		}

		spell, exists := spellMap[id]
		if exists {
			if spell.Formulas == nil {
				spell.Formulas = make(map[int][]core.CastFormula)
			}
			spell.Formulas[formula.CastLevel] = append(spell.Formulas[formula.CastLevel], formula)
			spellMap[id] = spell
		} else {
			s.Formulas = make(map[int][]core.CastFormula)
			s.Formulas[formula.CastLevel] = append(s.Formulas[formula.CastLevel], formula)
			spellMap[id] = s
		}
	}
