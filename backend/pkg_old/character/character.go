...
func NewCharacter(ctx context.Context, charConfig CharacterConfig) (*Character, error) {
	fmt.Printf("DEBUG: NewCharacter config for %s: ID=%d, ClassID=%d, RaceID=%d, Level=%d\n", charConfig.Name, charConfig.ID, charConfig.ClassID, charConfig.RaceID, charConfig.Level)
	if charConfig.ClassID < 1 || charConfig.ClassID > 13 {
		return nil, fmt.Errorf("invalid classData id during character initialization: %d", charConfig.ClassID)
	}
	if charConfig.RaceID < 1 || charConfig.RaceID > 9 {
	...
	char := Character{
		...
		HPConfig:             charConfig.HPConfig,
		Seed:                 charConfig.Seed,
		RNG:                  nil,
	}
	if char.HPConfig.Value == 0 && charConfig.HPConfig.Value != 0 {
		char.HPConfig.Value = charConfig.HPConfig.Value
	}
	if char.HPConfig.HPMethod == 0 && charConfig.HPConfig.HPMethod != 0 {
		char.HPConfig.HPMethod = charConfig.HPConfig.HPMethod
	}

	// Initialize managers
	// Roll Manager
	char.RollManager = initializeRollManager(&char, &char.Configuration)
	if charConfig.Seed.Seed1 != 0 || charConfig.Seed.Seed2 != 0 {
		char.RollManager.SetRNG(charConfig.Seed.Seed1, charConfig.Seed.Seed2)
	}
	...
	char.HPConfig.AmountToAdd = modifier
...
func NewCharacterWithRNG(ctx context.Context, charConfig CharacterConfig, rng *rand.Rand) (*Character, error) {
	fmt.Printf("DEBUG: NewCharacterWithRNG config for %s: ID=%d, ClassID=%d, RaceID=%d, Level=%d\n", charConfig.Name, charConfig.ID, charConfig.ClassID, charConfig.RaceID, charConfig.Level)
	if charConfig.ClassID < 1 || charConfig.ClassID > 13 {
		return nil, fmt.Errorf("invalid classData id during character initialization: %d", charConfig.ClassID)
	}
	if charConfig.RaceID < 1 || charConfig.RaceID > 9 {
...
func initializeSpellcastingManager(ctx context.Context, c *Character, knownSpells []int) (*spellcasting_manager.SpellcastingManager, error) {
	...
	if len(knownSpells) > 0 {
		spellData, err := spells.QuerySpellData(ctx, spells.SpellQueryParams{ID: knownSpells})
		if err != nil {
