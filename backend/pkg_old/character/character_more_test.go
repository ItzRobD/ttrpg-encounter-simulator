...
func TestExecuteAIRequest_RangedProducesDamage(t *testing.T) {
	...
	bow := &weapon.Weapon{
		...
		Properties: weapon.Properties{IsRanged: true, IsFinesse: false},
	}
...
func TestCreateWeaponAttackData_VersatileDie(t *testing.T) {
	...
	longsword := &weapon.Weapon{
		...
		Properties: weapon.Properties{IsRanged: false, IsVersatile: true},
	}
	...
	if !ad.IsVersatileAttack || ad.GetDie() != core.D10 {
		t.Errorf("expected versatile D10, got versatile=%v die=%v", ad.IsVersatileAttack, ad.GetDie())
	}
}
