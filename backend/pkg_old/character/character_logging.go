	...
	case events.EventType:
		...
		case events.ETRollEvent, events.ETRollInitiative:
			if res, ok := data.(*roll_manager.RollResult); ok {
				...
				events.LogDiceRollEventWithType(ctx, c, d.RollResult, d.DamageType, listener)
			}
		case events.ETAttackEvent:
			if res, ok := data.(*core.AttackResult); ok {
				events.LogMartialAttackEvent(ctx, c, res, nil, listener)
			}
		...
		case events.ETHPModifiedEvent:
			if d, ok := data.(*events.HPModifiedData); ok {
				events.LogHPModifiedEvent(ctx, c, d.Subject, d.Res, d.DamageType, d.SourceRollID, listener)
			}
		...
		case events.ETDamageModifiedEvent:
			if d, ok := data.(*events.DamageModifiedData); ok {
				events.LogDamageModifiedEvent(ctx, c, d.Subject, d.Res, d.DamageType, d.SourceRollID, listener)
			}
		...
		case events.ETEquipmentEvent:
			if d, ok := data.(*events.EquipmentEvent); ok {
				events.LogEquipmentEvent(ctx, c, d, listener)
			}
		case events.ETTurnStartEvent:
			events.LogTurnStartEvent(ctx, c, listener)
		}
	}
}
