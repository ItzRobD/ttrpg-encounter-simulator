...
func (ce *CombatEngine) turnStartEvents(combatantID int) error {
	...
	// Emit a single turn_start at the beginning of this turn/sequence
	// Generate a clean parent to anchor this turn and push context to all entities
	ce.EventContext.GenerateParentID()
	for _, c := range ce.Combatants {
		c.GetEntity().PushEventContext(ce.EventContext)
	}
	events.LogTurnStartEvent(ce.EventContext, entity, func(event interface{}) {
		// Forward through the simulation manager's dispatcher via entity listener
		if l := entity.GetEventListener(); l != nil {
			l(event)
		}
	})
...
func (ce *CombatEngine) executeLegendaryAction(actingCombatantID int) error {
	...
	for _, legID := range legCreatureIDs {
		...
		// Ensure the legendary creature has the current combat context before processing its turn
		_ = entity.UpdateAICombatContext(ce.CombatContext)
