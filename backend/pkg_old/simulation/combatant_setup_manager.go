...
func (csm *CombatantSetupManager) createMonstersFromConfigs(configs []monster.MonsterConfig) ([]*core.Combatant, []SetupError) {
	...
	for _, config := range configs {
		if csm.useHPAverageMonsters {
			config.HPMethod = core.HPSetAverage
		} else {
			config.HPMethod = core.HPSetRoll
		}
		...
		if err != nil {
			errors = append(errors, SetupError{
				Type:    "monster",
				ID:      config.Name,
				Message: fmt.Sprintf("Failed to create monster %s: %v", config.Name, err),
			})
			continue
		}
...
func (csm *CombatantSetupManager) createMonsters(ids []int) ([]*core.Combatant, []SetupError) {
	...
	for _, id := range ids {
		...
		if csm.useHPAverageMonsters {
			monsterConfig.HPMethod = core.HPSetAverage
		} else {
			monsterConfig.HPMethod = core.HPSetRoll
		}
		m, err := monster.NewMonsterWithRNG(csm.ctx, monsterConfig, csm.rng)
		if err != nil {
			errors = append(errors, SetupError{
				Type:    "monster",
				ID:      fmt.Sprintf("%d", monsterConfig.ID),
