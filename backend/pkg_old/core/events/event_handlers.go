...
func (h *UniversalEventHandler) handleMeleeAttack(e *MartialAttackEvent) {
	var s string
	dmgStr := fmt.Sprintf("%d %s", e.DamageTotal, e.DamageType)
	if e.DamageParts != "" {
		dmgStr = e.DamageParts
	}

	if e.CriticalHit {
		s = fmt.Sprintf("[Round %d] <Martial Critical Hit> Attack %d - %s attacks %s with %s. %d to hit, %d + %d. Target: %d. Success: %t. Damage: %s\n",
			e.GetRound(), e.AttackCount, e.GetActorName(), e.Target, e.AttackName, e.AttackRoll, e.AttackRoll, e.AttackModifier, e.TargetValue, e.Success, dmgStr)
	} else if !e.Success {
		...
		s = fmt.Sprintf("[Round %d] <Martial Attack> Attack %d - %s attacks %s with %s. %d to hit, %d + %d. Target: %d. Success: %t. Damage: %s\n",
			e.GetRound(), e.AttackCount, e.GetActorName(), e.Target, e.AttackName, e.AttackTotal, e.AttackRoll, e.AttackModifier, e.TargetValue, e.Success, dmgStr)
	}
	fmt.Print(s)
}
