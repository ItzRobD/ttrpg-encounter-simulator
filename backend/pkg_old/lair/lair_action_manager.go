...
func (lam *LairActionManager) ExecuteAdvanced(actionIndex int, primaryTarget core.Entity) ([]core.AttackResult, []core.Effect, error) {
	...
	case LAMDC:
		...
		for _, comp := range dmgRoll.DamageComponents {
			lam.parent.LogEvent(events.ETRollEvent, &events.DiceRollData{
				RollResult: &roll_manager.RollResult{
					DiceRollType:   dmgRoll.DiceRollType,
					FinalRollValue: comp.RollValue,
					FinalRolls:     comp.DiceRolls,
					Modifier:       comp.Modifier,
					Total:          comp.Total,
		...
		for _, t := range targets {
			...
			saveRes, err := t.MakeSavingThrow(a.DCAbility, a.DCValue, false, core.DamageType(a.AttackData.GetDamageType()), simOpts)
			if err != nil {
				return nil, nil, err
			}
			// Log DC event (basic)
			...
			ar := core.AttackResult{ // reuse for summary
				...
				AttackName: a.Name,
				DamageRoll: dmgRoll,
				DamageType: core.DamageType(a.AttackData.GetDamageType()),
				IsHit:      applied > 0,
			}
			results = append(results, ar)
			if applied > 0 {
				effects = append(effects, core.Effect{Type: core.EffectDamage, Value: applied, DamageType: core.DamageType(a.AttackData.GetDamageType())})
			}
		}
	}
